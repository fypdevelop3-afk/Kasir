...
import { 
  ...
  Copy, Check, Wifi, Download, Upload
} from "lucide-react";
...
export const AdminPanel: React.FC = () => {
  ...
  const handleExportBackup = () => {
    try {
      ...
      const storeNameClean = (storeSettings.storeName || "pos-umkm").toLowerCase().replace(/[^a-z0-9]/g, "-");
      const dateStr = new Date().toISOString().split("T")[0];
      downloadAnchor.setAttribute("download", `cadangan-kasir-${storeNameClean}-${dateStr}.json`);
      document.body.appendChild(downloadAnchor);
      downloadAnchor.click();
      downloadAnchor.remove();
      
      setSyncSuccessMessage("File Cadangan (.json) berhasil diunduh! Simpan file ini di tempat aman.");
      setSyncErrorMessage(null);
    } catch (err: any) {
      setSyncErrorMessage("Gagal membuat data cadangan: " + err.message);
      setSyncSuccessMessage(null);
    }
  };
  ...
  const handleImportBackup = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    fileReader.onload = async (e) => {
      try {
        ...
        const confirmMsg = "⚠️ PERINGATAN KRITIS: Tindakan ini akan menghapus dan menimpa seluruh data jualan aktif Anda di HP ini dengan isi file cadangan.\n\nApakah Anda sangat yakin ingin melanjutkan?";
        if (window.confirm(confirmMsg)) {
          ...
          if (ok) {
            setSyncSuccessMessage("🎉 Berhasil dipulihkan! Semua data produk, transaksi, kategori, s/d pengaturan telah berhasil dipulihkan secara lokal.");
          } else {
            setSyncErrorMessage("File cadangan tidak valid atau gagal diproses.");
          }
        }
      } catch (err: any) {
        setSyncErrorMessage("Format file cadangan tidak valid (file .json Anda kemungkinan rusak): " + err.message);
      } finally {
        setIsConnecting(false);
      }
    };
